...
import { OutletContext, Scrapper, ScrapperStatus, ScrapperTarget as ScrapperTargetType } from '../types';
...
const ScrapperTarget: FunctionComponent<Props> = function ({ target, onRun }) {
    ...
    const scrapperOptions: OptionsMenuItem[] = scrappers
        .filter(s => [ScrapperStatus.NEW, ScrapperStatus.IDDLE].includes(s.status))
        .map(s => ({
            label: s.name,
    ...
    return (
        <div className="rounded overflow-hidden shadow-lg bg-white">
            ...
            <div className="text-xl mb-2 flex justify-between">
                <span className="text-ellipsis font-bold">{ target.name }</span>
